...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    new_devices.append(VoltageSensor(hub.inverter))

    if new_devices:
        async_add_entities(new_devices)


class VoltageSensor(Entity):
    ...
    def __init__(self, inverter: Inverter):
        ...
        self._attr_unique_id = f"{self._inverter.inverter_id}_voltage"
        self._attr_name = f"{self._inverter.name} Voltage"

    @property
    def device_info(self):
        """Return information to link this entity with the correct device."""
        return self._inverter.device_info

    @property
    def available(self) -> bool:
        """Return True if roller and hub is available."""
        return self._inverter.online
